...
run lambda { |env|
  ...
  body = File.read('index.html').gsub(/\[OVERALL_RATIO\]/, "#{sprintf("%.2f", beard_ratio(members))}/#{members.length}")
  body.gsub!(/\[OFFICE_RATIO\]/, "#{sprintf("%.2f", beard_ratio(in_office))}/#{in_office.length}")
